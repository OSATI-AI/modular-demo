(function() {
  ...
  tailwindLink.setAttribute('href', 'https://cdnjs.cloudflare.com/ajax/libs/tailwindcss/2.2.16/tailwind.min.css');
  ...
  // Inject the HTML into shadow DOM
  shadow.innerHTML += `
    ...
    <div id="chat-popup" class="hidden absolute bottom-20 right-0 w-96 bg-white rounded-md shadow-md flex flex-col transition-all text-sm">
      <div id="chat-header" class="flex justify-between items-center p-4 bg-gray-800 text-white rounded-t-md">
        <h3 class="m-0 text-lg">AI Tutor</h3>
  ...
  window.addEventListener('apiResponse', function(event) {
    reply(event.detail);
  });
  

  function triggerMessageEvent(message) {
    const event = new CustomEvent('userMessage', { detail: message });
